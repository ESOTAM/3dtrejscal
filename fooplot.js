...
theplot1.addPlot(String(varst),FOOPLOT_TYPE_FUNCTION);
...
if(isNaN(y)){
  ...
  fx1.push(this.vars.x);
  fy1.push(y);
}
      /*  fy1.push(y);
 fx1.push(this.vars.x);

     // ylinea(this.vars.x,y);
fy1[contarmas]=0;
...
fx1[contarmas]=0;*/
      ...
      if(isNaN(y)) {
        if(!lastIsNaN) {
          if(lastychange>0 && lasty>0) {
           ;this.context.lineTo(lastpx,0);
           // fy1[contarmas]=0;
           // fx1[contarmas]=lastpx;

          }
          else if(lastychange<0 && lasty<0) {
            this.context.lineTo(lastpx,this.height);
           //  fy1[contarmas]=this.height;
            //fx1[contarmas]=lastpx0;
        }
